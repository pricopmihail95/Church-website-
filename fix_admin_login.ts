...
if (!admin.includes("import { auth } from '../firebase'")) {
  admin = admin.replace(
...
if (!admin.includes('const [email, setEmail] = useState')) {
  admin = admin.replace(
...
admin = admin.replace(
  /<form onSubmit=\{handleLogin\} className="space-y-6">[\s\S]*?<\/form>/m,
  formReplacement
);
